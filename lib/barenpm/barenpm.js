...
var bpm = {
        ...
        if (err) {
          ...
          path.exists(tmpdir + 'package/package.json', function(exists) {
             if (exists) {
               ...
               fs.readdir(tmpdir, function(files) {
                 if (files && files[0]) {
    ...
    bpm.getProject(function(project) {
      bpm.installModule(name, function (err, suc) {
        if (err) {
          ...
          //if (bpm._pkglist.length <= 0) {
            cb(null, name + ' Installed');
          //}
        }
      });
    });
  ...
  installPackage: function(pkg, tmpdir, cb) {
    ...
    bpm.moveDirToLibs(tmpdir, pkgdir, function(err, succ) {
      pkg.modules = pkg.modules || {};
      //bpm.linkFile(libdir + 'index.js','./.bpm');
      ...
      if (libdir) {
        var file = fs.readdir(bpmdir + libdir, function(files) {
          files.forEach(function(file) {
            fs.stat(bpmdir + libdir + '/' + file, function(err, stat) {
              if (stat.isFile()) {
      ...
      modules.forEach(function(from) {
        var to = bpmdir + pkg.modules[from];
        var validTo = false;
        function checkTo() {
          try {
            fs.statSync(to);
        ...
        if (validTo) {
          ...
          exec("mkdir -p '" + bpmdir + fromBase + "'", function(err, stdout) {
            bpm.linkFile(bpmdir + from, to)
          });
        }
      });
